...
Read `../add-bookshelf-book/references/book-schema.md`. Before running the
bookshelf script, create a dedicated shelf-cover image from the article cover:

1. Inspect the article cover with `view_image`.
2. Invoke the `imagegen` skill and built-in `image_gen` tool in image-to-image
   mode, using that cover as the reference image. The required model is GPT
   Image 2; if GPT Image 2 or the built-in image edit path is unavailable, stop
   instead of substituting a crop, resize, direct copy, CSS treatment, or a
   different generator.
3. Redesign the reference as finished vertical book-cover artwork that keeps
   the source subject and editorial meaning while fitting the Roland bookshelf.
   Do not bake the article title, logos, signatures, or watermarks into the
   bitmap because the exact title remains live DOM text.
4. Inspect the generated result, then copy the selected generated file from
   `$CODEX_HOME/generated_images/` into a temporary project-local working path.
   Record the final prompt and generation mode. The generated bytes must differ
   from the article cover.

Then dry-run and add the reviewed article, passing the generated image to both
commands:
...
python3 .agents/skills/publish-high-exposure-articles/scripts/add_article_to_bookshelf.py \
  ...
  --generated-cover <project-local-img2-output> --dry-run
python3 .agents/skills/publish-high-exposure-articles/scripts/add_article_to_bookshelf.py \
  --root /Users/garylau/Work/rolandwayne \
  --article content/blog/x-<status-id>.md \
  --generated-cover <project-local-img2-output>
```
...
- refuse a new book unless `--generated-cover` is a valid img2img output whose
  path and bytes differ from the article cover, then copy that generated asset to
...
Require the generated article page, `dist/articles/index.html`, all article assets, the generated book cover,
...
book number, GPT Image 2 prompt/mode, generated cover, validation results,
commit, and push result. A
...
- `scripts/add_article_to_bookshelf.py` — dry-run or append its complete React
  shelf book from a required, independently generated img2img cover.
